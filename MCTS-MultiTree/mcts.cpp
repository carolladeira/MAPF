...
#define AG 2 // se -1 imprime todos os agentes
#define MCSEARCH
#define EXPAND
...
void monte_carlo::check_goalAndColision(multi_tree *tp, int a, multi_agent *map) {
    ...
    for (int i = 0; i < list_agents.size(); i++) {
        for (int j = 0; j < list_agents[i].size(); j++) {
            ...
            if (x == map->goal_vec.at(i).agent_x && y == map->goal_vec.at(i).agent_y) {
                goal_found[i] = true;
        ...
        for (int j = 0; j < list_agents[i].size(); j++) {
            ...
            for (int j = 0; j < list_agents[i + 1].size(); j++) {
                ...
                if (x_A == x_B && y_A == y_B) { // colisao de vertex
                    cout << "Colisao -  Agente: " << i << " e " << i + 1 << " no tempo " << j << " posicao " << x_A
                         << "," << y_A << endl;
                    break;
                }
            }
...
void monte_carlo::update_tree(multi_tree *tp, int level, int n, int agent) {
    int a = agent;
    //update no raiz (level 0), o no do level 1 vira raiz (level 0)
    tp->ag_tree.at(a).tree_vec.at(0).level_vec.at(0) = tp->ag_tree.at(a).tree_vec.at(level).level_vec.at(n);
    vector<int> parents;
    ...
    tp->ag_tree.at(a).tree_vec.erase(tp->ag_tree.at(a).tree_vec.begin() + 1);
    ...
    for (int lev = 1; lev < tp->ag_tree.at(a).tree_vec.size(); lev++) {
        for (int en = 0; en < tp->ag_tree.at(a).tree_vec.at(lev).level_vec.size(); en++) {
            ...
            if (std::find(parents.begin(), parents.end(), p_num) != parents.end()) {
                ...
                tp->ag_tree.at(a).tree_vec.at(lev).level_vec.erase(
                        tp->ag_tree.at(a).tree_vec.at(lev).level_vec.begin() + en);
                en--;
            }
...
void monte_carlo::create_root_nodes(multi_tree *tp, multi_agent *map) {
    ...
    for (int ag = 0; ag < map->n_agents; ag++) { //Create Root Nodes
        ...
        tp->create_node(0, n_nodes, map->agent_vec.at(ag).agent_x, map->agent_vec.at(ag).agent_y, ag, node_number, 0,
                        -1); //Root Node
    }
}
...
void monte_carlo::rollout(multi_tree *tp, multi_agent *map, int n) { //n = node position in level of tree
    ...
    //manhattan distance
    int xgoal = map->goal_vec.at(a_num).agent_x;
    int ygoal = map->goal_vec.at(a_num).agent_y;
    double dist_ini = std::abs(x - xgoal) + std::abs(y - ygoal);
    if (dist_ini == 0) dist_ini = rollout_c_goal; //agente esta na posicao final
    else dist_ini = 1 / dist_ini;
    ...
    while (a < rollout_iterations) {
        ...
        for (int i = 0; i < rollout_steps; i++) {
            ...
            if (x == map->goal_vec.at(a_num).agent_x && y == map->goal_vec.at(a_num).agent_y) {
        ...
        if (dist == 0) dist = 1;
        dist = 1 / dist;
        dist_fim = dist_fim + dist;
        a++;
    }
#ifdef ROLLOUT
    if (a_num == AG || AG == -1) {

    cout <<endl<< "Node: " << tp->ag_tree.at(a_num).tree_vec.at(lev).level_vec.at(n).n_number << " q: " << q_val << " dist_ini: "
         << dist_ini << " dist_fim "<< dist_fim << " q_value: " << q_val + dist_ini + dist_fim
         << endl;
    }
#endif
    tp->ag_tree.at(a_num).tree_vec.at(lev).level_vec.at(n).q_node = q_val + dist_ini + dist_fim;
}
